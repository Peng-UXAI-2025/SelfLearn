...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function initializeApp() {
    ...
    `;
      ...
      // Cancel button
      modal
        .querySelector("#cancel-api-settings-btn")
        .addEventListener("click", function () {
          modal.remove();
        });
      ...
      // Save button
      modal
        .querySelector("#save-api-keys-btn")
        .addEventListener("click", function () {
          const newOpenaiKey = document
            .getElementById("openai-key")
            .value.trim();
          const newGeminiKey = document
  ...
  function showClipboardCopilotWindow() {
    ...
    // Create window
    const clipboardWindow = document
      .getElementById("clipboard-copilot-window")
      .cloneNode(true);
  ...
  function initializeClipboardCopilotWindow(windowElement) {
    ...
    if (monitoringBtn) {
      monitoringBtn.addEventListener("click", function () {
        if (!app.clipboardMonitorActive) {
          ...
          // Update status
          const statusElement =
            windowElement.querySelector(".clipboard-status");
          statusElement.innerHTML =
            "<p>Clipboard monitoring is active. Copy text from anywhere to process it.</p>";
          ...
          const statusElement =
            windowElement.querySelector(".clipboard-status");
          statusElement.innerHTML =
            '<p>Clipboard monitoring is inactive. Click "Start Monitoring" to begin capturing copied text.</p>';
    ...
    if (customProcessBtn) {
      customProcessBtn.addEventListener("click", function () {
        const customPrompt =
          windowElement.querySelector("#custom-prompt").value;
        const textElement = windowElement.querySelector("#captured-text");
        const text = textElement ? textElement.textContent : "";

        if (text.trim() && customPrompt.trim()) {
          window.textProcessor.processTextWithCustomPrompt(
            text,
            customPrompt,
            windowElement
          );
